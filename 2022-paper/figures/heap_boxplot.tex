...
\node[right = 0.2cm of threads c1r4.east,anchor=north,rotate=90] {\num{64} Threads};\end{tikzpicture}
  \caption{Relative run-time changes to minimal memory configuration, by heap memory. The available heap memory has only a marginal impact on the runtime, with large amounts of available heap memory degrading performance independently of the number of threads used.}
